...
const MultiFileDropzone = React.forwardRef<HTMLInputElement, InputProps>(
  ...
  {/* Contêiner principal: Dropzone + Lista de Arquivos */}
  <div className="flex w-full max-w-3xl gap-4">
    {/* Dropzone ocupa espaço inteiro se não houver arquivos, caso contrário ocupa 1/3 */}
    <div className={twMerge("transition-all", value?.length ? "w-1/3" : "w-full")}>
      <div {...getRootProps({ className: dropZoneClassName })}>
        <input ref={ref} {...getInputProps()} />
        <div className="flex flex-col items-center justify-center text-xs text-gray-400">
          <Upload className="mb-1 h-4 w-4" />
          <div className="text-gray-400">Arraste ou clique para selecionar.</div>
        </div>
      </div>
      {/* Mensagem de erro */}
    ...
    {/* Lista de arquivos aparece apenas se houver arquivos */}
    {(value?.length ?? 0) > 0 && (
      <div className="max-h-40 w-2/3 overflow-y-auto">
            ...
            <div className="flex items-center gap-2 text-gray-500 dark:text-white">
              ...
              <div className="flex w-12 justify-end text-xs">
                {progress === 'PENDING' ? (
                  ...
                  >
                    <Trash2Icon className="shrink-0" size={15} />
                  </button>
                ) : progress === 'ERROR' ? (
                  ...
                  <CircleCheck className="shrink-0 text-green-600" size="20" />
                )}
              </div>
            </div>
            {/* Barra de progresso */}
